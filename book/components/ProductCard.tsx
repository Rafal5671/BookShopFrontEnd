...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const { name, image, price, discountedPrice } = product;
  const router = useRouter();

  // Use product image if available, otherwise use placeholder
  const imageUrl = "https://via.placeholder.com/300x200.png?text=Brak+zdjęcia";
  ...
  return (
    <Card className="max-w-sm shadow-lg hover:shadow-2xl transition-shadow duration-300 bg-primary-100">
      ...
      <CardFooter className="flex justify-center pt-0 pb-4">
        <Button className="bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white font-bold py-2 px-6 rounded-full shadow-md flex items-center gap-2 transition-colors duration-300">
